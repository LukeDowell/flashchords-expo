...
import {ChordSymbol, ModifierContext, Stave, TickContext} from "vexflow4";
...
export default function DiatonicChordExercise({musicKey, onEnd, options}: Props) {
  const { context, width, height } = useVexflowContext()
  ...
  useInterval(() => {
    ...
    const matrix = new WebKitCSSMatrix(window.getComputedStyle(staveGroup).transform)
    // Why on earth is it .m41 for X, I don't know anything about matrices
    // nice https://stackoverflow.com/questions/4868678/id-like-to-know-how-webkitcssmatrix-actually-works
    // from SO: '.m41' is the X axis (4th row 1st col) and '.m42'(4th row 2nd col) the Y axis
    const currentMeasureIndex = Math.floor(matrix.m41 * -1 / STAVE_WIDTH)
...
const VexflowOutput = styled.View`
    overflow: hidden;
`
