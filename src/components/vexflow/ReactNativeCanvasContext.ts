...
import { RenderContext, TextMeasure } from 'vexflow';
import { globalObject, warn } from 'vexflow';
 ...
 */
export class ReactNativeCanvasContext extends RenderContext {
  ...
  static get CANVAS_BROWSER_SIZE_LIMIT(): number {
    return 32767; // FIXME does this apply for react native
  }
  ...
  constructor(canvas: Canvas, context: CanvasRenderingContext2D) {
    super();
  ...
  openRotation(angleDegrees: number, x: number, y: number) {
    // FIXME is this needed in native?
  ...
  resize(width: number, height: number, devicePixelRatio?: number): this {
    ...
    [width, height] = ReactNativeCanvasContext.sanitizeCanvasDims(width * dpr, height * dpr);
